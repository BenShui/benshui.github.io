---
layout: post
title: Pre-FOMC Announcement Drift
date: 2026-05-30 12:00:00-0700
description: The disappearing pre-FOMC announcement drift
tags: asset-pricing
categories: asset-pricing
---

Pre-FOMC drift replicates and is roughly halved post-2011. On SPY minute bars 2000–2026 the average 09:30 (day t−1) → 14:15 (day t) drift is +36 bps (n=209 scheduled meetings); splitting at 2012-01-01 gives +48 bps pre vs +25 bps post. The break aligns with Lucca & Moench's NY Fed staff report (2011).


{% include figure.liquid loading="eager" path="files/Misc/pre_fomc_drift_spy_full.png" title="2000-2026" class="img-fluid rounded z-depth-1" %}
{% include figure.liquid loading="eager" path="files/Misc/pre_fomc_drift_spy_lm.png" title="2000-2011" class="img-fluid rounded z-depth-1" %}
{% include figure.liquid loading="eager" path="files/Misc/pre_fomc_drift_spy_postlm.png" title="2011-2026" class="img-fluid rounded z-depth-1" %}